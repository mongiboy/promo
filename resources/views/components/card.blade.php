...
<a class="flex flex-col basis-full md:basis-1/2 lg:basis-1/3 border-black/10 shadow-lg transition duration-300 ease-in-out hover:shadow-xl rounded-lg bg-white py-9 px-4 lg:px-9 offer"
   href="{{ route('offer.redirect', $offer) }}" rel="nofollow sponsored" {{ $offer->promocode ? 'data-id='.$offer->id : 'target="_blank"' }}>
    @if($showLogo)
